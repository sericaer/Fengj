...
{
    class BerlinBuilder : HeightMapBuilder
    {
        ...
        {
            var positions = Enumerable.Range(0, endIndex - startIndex).SelectMany(x => Enumerable.Range(0, endIndex - startIndex).Select(y => (x, y)));
            ...
            for (int x = startIndex - 10; x < endIndex + 10; x += 10)
            {
                ...
                {
                    grandDict.Add((x, y), (float)random.NextDouble());
                }
            }
